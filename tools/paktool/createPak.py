...
# Calculate and add the size of the header
headerLength = 0
for f in files:
    headerLength += len(f) + 10
buffer.write(intToBytes(headerLength, 4))

# Write the files header
filePosition = headerLength + 32 + 8 # 32 is the lenght of the main header
for f in files:

    # TODO
    # Write the path lenght
    buffer.write(intToBytes(len(f), 1))
    buffer.write(intToBytes(0, 1))

    #TODO is this linux specific?
    # Write file path and name
    buffer.write(f.replace("/", "\\").encode('utf-8'))
